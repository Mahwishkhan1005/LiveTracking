...
const NotificationsPage = () => {
  ...
  const handleNotificationPress = async (item: any) => {
    // 1. Mark as read in the database/UI
    if (!item.read) {
      await markNotificationAsRead(item.id);
      loadNotifications(); // Refresh list to update the unread styling
    }

    // 2. Extract the internal path from the full URL
    // Example: "http://192.168.0.192:8081/CurrentOrder?orderId=171..." -> "/CurrentOrder?orderId=171..."
    if (item.link) {
      let targetPath = item.link;

      if (targetPath.startsWith("http")) {
        try {
          // Use the URL object to extract just the path and query parameters
      ...
      // 3. Navigate to the specific order page
      // Expo Router will automatically match "/CurrentOrder" to "app/(USER)/CurrentOrder.tsx"
